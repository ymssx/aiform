...
export function buildFillPrompt(simplifiedDOM, userProfile, memories, userSupplement, pageContext, options = {}) {
  ...
  // 快速模式：极简 prompt，跳过详细规则说明，让 AI 直接返回结果
  if (isQuickMode) {
    return `分析以下表单DOM，结合用户信息，直接返回JSON填充指令。不要思考，不要推理，直接输出结果。
...
${userSupplement?.includes?.('[AI_GENERATE]') ? '【必须为每个字段都生成内容，禁止返回null】' : ''}

## 输出格式（严格JSON）
{"fields":[{"selector":"CSS选择器","label":"字段名","value":"值","type":"text|select|textarea|checkbox|radio","options":[]}],"updatedProfile":{},"newMemories":[],"changeLog":[]}

selector优先用[name]、#id，其次用精确CSS路径。select的value必须从options中选。`;
  }
